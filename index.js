...
const DEFAULT_STATE = { points: {}, warnings: {}, tickets: {}, ticketMeta: {}, closedTickets: {}, suggestions: {}, giveaways: {}, reminders: {}, pointLog: [], counters: { support: 0, purchase: 0 }, afk: {}, protection: { spam: {}, joins: [] } };
const state = fs.existsSync(DATA) ? { ...DEFAULT_STATE, ...JSON.parse(fs.readFileSync(DATA, 'utf8')) } : DEFAULT_STATE;
state.counters ||= { support: 0, purchase: 0 };
state.ticketMeta ||= {}; state.closedTickets ||= {}; state.suggestions ||= {}; state.giveaways ||= {}; state.reminders ||= {}; state.pointLog ||= []; state.afk ||= {}; state.protection ||= { spam: {}, joins: [] };
...
const commands = [
  ...
  { name: 'userinfo', description: 'معلومات عضو', options: [{ name: 'user', description: 'العضو', type: 6 }] },
  { name: 'serverinfo', description: 'معلومات السيرفر' },
  { name: 'avatar', description: 'عرض صورة العضو', options: [{ name: 'user', description: 'العضو', type: 6 }] },
  { name: 'banner', description: 'عرض بانر عضو', options: [{ name: 'user', description: 'العضو', type: 6 }] },
  { name: 'roleinfo', description: 'معلومات رتبة', options: [{ name: 'role', description: 'الرتبة', type: 8, required: true }] },
  { name: 'channelinfo', description: 'معلومات روم', options: [{ name: 'channel', description: 'الروم', type: 7 }] },
  { name: 'emojiinfo', description: 'معلومات إيموجي', options: [{ name: 'emoji', description: 'الإيموجي أو آيديه', type: 3, required: true }] },
  { name: 'membercount', description: 'عدد الأعضاء' },
  { name: 'servericon', description: 'أيقونة السيرفر' },
  { name: 'nickname', description: 'تغيير لقب عضو', options: [{ name: 'user', description: 'العضو', type: 6, required: true }, { name: 'name', description: 'اللقب الجديد', type: 3 }] },
  { name: 'move', description: 'نقل عضو لروم صوتي', options: [{ name: 'user', description: 'العضو', type: 6, required: true }, { name: 'channel_id', description: 'آيدي الروم الصوتي', type: 3, required: true }] },
  { name: 'voicekick', description: 'إخراج عضو من الصوت', options: [{ name: 'user', description: 'العضو', type: 6, required: true }] },
  { name: 'voicemute', description: 'ميوت صوتي', options: [{ name: 'user', description: 'العضو', type: 6, required: true }] },
  { name: 'voiceunmute', description: 'إلغاء ميوت صوتي', options: [{ name: 'user', description: 'العضو', type: 6, required: true }] },
  { name: 'lock', description: 'قفل الروم' },
  { name: 'unlock', description: 'فتح الروم' },
  { name: 'clear', description: 'حذف رسائل', options: [{ name: 'amount', description: 'العدد', type: 4, required: true }] },
  { name: 'slowmode', description: 'تغيير التباطؤ', options: [{ name: 'seconds', description: 'الثواني', type: 4, required: true }] },
  { name: 'hide', description: 'إخفاء الروم' },
  { name: 'unhide', description: 'إظهار الروم' },
  { name: 'announce', description: 'إعلان احترافي', options: [{ name: 'message', description: 'النص', type: 3, required: true }, { name: 'channel', description: 'الروم', type: 7 }] },
  { name: 'embed', description: 'إنشاء إيمبد', options: [{ name: 'title', description: 'العنوان', type: 3, required: true }, { name: 'description', description: 'الوصف', type: 3, required: true }] },
  { name: 'poll', description: 'تصويت', options: [{ name: 'question', description: 'السؤال', type: 3, required: true }] },
  { name: 'giveaway', description: 'بدء قيف أواي', options: [{ name: 'prize', description: 'الجائزة', type: 3, required: true }, { name: 'winners', description: 'عدد الفائزين', type: 4 }, { name: 'minutes', description: 'المدة بالدقائق', type: 4 }] },
  { name: 'reroll', description: 'إعادة سحب قيف أواي', options: [{ name: 'message_id', description: 'آيدي رسالة القيف أواي', type: 3, required: true }] },
  { name: 'afk', description: 'تفعيل AFK', options: [{ name: 'reason', description: 'السبب', type: 3 }] },
  { name: 'remind', description: 'تذكير', options: [{ name: 'minutes', description: 'بعد كم دقيقة', type: 4, required: true }, { name: 'text', description: 'التذكير', type: 3, required: true }] },
  { name: 'leaderboard', description: 'أفضل الإداريين بالنقاط' },
  { name: 'resetpoints', description: 'تصفير النقاط', options: [{ name: 'scope', description: 'النطاق', type: 3, choices: [{ name: 'all', value: 'all' }, { name: 'weekly', value: 'weekly' }, { name: 'monthly', value: 'monthly' }] }] },
  ...
  { name: 'ticket-unclaim', description: 'إلغاء استلام التذكرة' },
  { name: 'ticket-transfer', description: 'نقل التذكرة إلى نوع آخر', options: [{ name: 'type', description: 'النوع الجديد', type: 3, required: true, choices: [{ name: 'دعم', value: 'support' }, { name: 'شراء', value: 'purchase' }] }] },
...
async function uploadFile(channelId, filename, content, mime, payload = {}) {
  const form = new FormData();
  form.append('payload_json', JSON.stringify(payload));
  form.append('files[0]', new Blob([content], { type: mime }), filename);
...
function transcriptHtml(text, title) {
  const safe = String(text || '').replace(/[&<>]/g, c => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;' }[c]));
  return `<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><title>${title}</title><style>body{background:#0f1020;color:#f8fafc;font-family:Arial;padding:24px}.box{background:#181a2e;border:1px solid #7c3aed;border-radius:16px;padding:18px;white-space:pre-wrap}</style></head><body><h1>${title}</h1><div class="box">${safe}</div></body></html>`;
}
...
async function command(i) {
  ...
  if (['userinfo','serverinfo','avatar','banner','roleinfo','channelinfo','emojiinfo','membercount','servericon'].includes(name)) return infoCommand(i, name);
  if (['nickname','move','voicekick','voicemute','voiceunmute','lock','unlock','clear','slowmode','hide','unhide','announce','embed','poll','giveaway','reroll','afk','remind','resetpoints'].includes(name)) return utilityCommand(i, name);
...
async function fetchUser(userId) { return api('GET', `/users/${userId}`).catch(() => null); }
function userAvatar(user) { return user?.avatar ? `https://cdn.discordapp.com/avatars/${user.id}/${user.avatar}.png?size=1024` : null; }
async function infoCommand(i, name) {
  const userId = opt(i, 'user') || i.member.user.id;
  if (name === 'userinfo' || name === 'avatar' || name === 'banner') {
    const user = await fetchUser(userId);
    const e = makeEmbed(name === 'userinfo' ? '👤 معلومات العضو' : name === 'avatar' ? '🖼️ صورة العضو' : '🎨 بانر العضو', `**العضو:** <@${userId}>
**الآيدي:** \`${userId}\`
**تاريخ إنشاء الحساب:** <t:${Math.floor(((BigInt(userId) >> 22n) + 1420070400000n) / 1000n)}:F>`, COLORS.info);
    const avatar = userAvatar(user);
    if (avatar) e.image = { url: avatar };
    return reply(i, { embeds: [e] }, false);
  }
  if (name === 'serverinfo') return reply(i, { embeds: [makeEmbed('🏰 معلومات السيرفر', `**السيرفر:** ${config.serverName}
**Guild ID:** \`${i.guild_id}\``, COLORS.main)] }, false);
  if (name === 'membercount') return reply(i, { embeds: [makeEmbed('👥 عداد الأعضاء', 'يظهر رقم العضو في رسالة الترحيب، ويمكن ربط روم إحصائيات لاحقًا من config.', COLORS.info)] }, false);
  if (name === 'servericon') return reply(i, { embeds: [makeEmbed('🖼️ أيقونة السيرفر', 'استخدم إعدادات السيرفر أو ضع رابط الشعار في `serverBanner`.', COLORS.info)] }, false);
  if (name === 'roleinfo') return reply(i, { embeds: [makeEmbed('🎭 معلومات رتبة', `الرتبة: <@&${opt(i, 'role')}>
الآيدي: \`${opt(i, 'role')}\``, COLORS.info)] }, false);
  if (name === 'channelinfo') return reply(i, { embeds: [makeEmbed('📌 معلومات روم', `الروم: <#${opt(i, 'channel') || i.channel_id}>
الآيدي: \`${opt(i, 'channel') || i.channel_id}\``, COLORS.info)] }, false);
  if (name === 'emojiinfo') return reply(i, { embeds: [makeEmbed('😀 معلومات إيموجي', `القيمة: ${opt(i, 'emoji')}`, COLORS.info)] }, false);
}
async function utilityCommand(i, name) {
  const needsChat = ['lock','unlock','clear','slowmode','hide','unhide','announce','embed','poll','giveaway','reroll'];
  if (needsChat.includes(name) && !hasRole(i.member, 'Chat Manager')) return reply(i, { embeds: [makeEmbed('صلاحية غير كافية', 'تحتاج رتبة Chat Manager.', COLORS.danger)] });
  if (['nickname','move','voicekick','voicemute','voiceunmute'].includes(name) && !hasRole(i.member, 'Role Manager')) return reply(i, { embeds: [makeEmbed('صلاحية غير كافية', 'تحتاج رتبة Role Manager.', COLORS.danger)] });
  if (name === 'afk') { state.afk[i.member.user.id] = { reason: opt(i, 'reason') || 'AFK', at: Date.now() }; save(); return reply(i, { embeds: [makeEmbed('💤 AFK', `تم تفعيل AFK: ${state.afk[i.member.user.id].reason}`, COLORS.info)] }); }
  if (name === 'remind') { const ms = opt(i, 'minutes') * 60000; const text = opt(i, 'text'); setTimeout(() => post(i.channel_id, { content: `<@${i.member.user.id}>`, embeds: [makeEmbed('⏰ تذكيرك', text, COLORS.gold)] }).catch(console.error), ms); return reply(i, { embeds: [makeEmbed('⏰ تم ضبط التذكير', `بعد **${opt(i, 'minutes')}** دقيقة.`, COLORS.ok)] }); }
  if (name === 'resetpoints') { state.points = {}; state.pointLog = []; save(); return reply(i, { embeds: [makeEmbed('♻️ Reset Points', 'تم تصفير النقاط.', COLORS.ok)] }); }
  if (name === 'nickname') { await api('PATCH', `/guilds/${i.guild_id}/members/${opt(i, 'user')}`, { nick: opt(i, 'name') || null }); return reply(i, { embeds: [makeEmbed('✏️ تم تغيير اللقب', `العضو: <@${opt(i, 'user')}>`, COLORS.ok)] }); }
  if (name === 'move') { await api('PATCH', `/guilds/${i.guild_id}/members/${opt(i, 'user')}`, { channel_id: opt(i, 'channel_id') }); return reply(i, { embeds: [makeEmbed('🔊 تم النقل', `العضو: <@${opt(i, 'user')}>`, COLORS.ok)] }); }
  if (name === 'voicekick') { await api('PATCH', `/guilds/${i.guild_id}/members/${opt(i, 'user')}`, { channel_id: null }); return reply(i, { embeds: [makeEmbed('🔇 Voice Kick', `تم إخراج <@${opt(i, 'user')}> من الصوت.`, COLORS.ok)] }); }
  if (name === 'voicemute' || name === 'voiceunmute') { await api('PATCH', `/guilds/${i.guild_id}/members/${opt(i, 'user')}`, { mute: name === 'voicemute' }); return reply(i, { embeds: [makeEmbed('🎙️ Voice Moderation', `تم تنفيذ الأمر على <@${opt(i, 'user')}>.`, COLORS.ok)] }); }
  if (name === 'clear') { const msgs = await api('GET', `/channels/${i.channel_id}/messages?limit=${Math.min(100, Math.max(1, opt(i, 'amount')))}`); await api('POST', `/channels/${i.channel_id}/messages/bulk-delete`, { messages: msgs.map(m => m.id) }); return reply(i, { embeds: [makeEmbed('🧹 Clear', 'تم حذف الرسائل.', COLORS.ok)] }); }
  if (name === 'lock' || name === 'unlock') { await api('PUT', `/channels/${i.channel_id}/permissions/${i.guild_id}`, { type: 0, deny: name === 'lock' ? PERMS.SendMessages.toString() : '0', allow: name === 'unlock' ? PERMS.SendMessages.toString() : '0' }); return reply(i, { embeds: [makeEmbed(name === 'lock' ? '🔒 Lock' : '🔓 Unlock', 'تم تنفيذ الأمر.', COLORS.ok)] }); }
  if (name === 'hide' || name === 'unhide') { await api('PUT', `/channels/${i.channel_id}/permissions/${i.guild_id}`, { type: 0, deny: name === 'hide' ? PERMS.ViewChannel.toString() : '0', allow: name === 'unhide' ? PERMS.ViewChannel.toString() : '0' }); return reply(i, { embeds: [makeEmbed(name === 'hide' ? '🙈 Hide' : '👁️ Unhide', 'تم تنفيذ الأمر.', COLORS.ok)] }); }
  if (name === 'slowmode') { await api('PATCH', `/channels/${i.channel_id}`, { rate_limit_per_user: opt(i, 'seconds') }); return reply(i, { embeds: [makeEmbed('🐢 Slowmode', `تم ضبطه إلى **${opt(i, 'seconds')}** ثانية.`, COLORS.ok)] }); }
  if (name === 'announce') { await post(opt(i, 'channel') || i.channel_id, { embeds: [makeEmbed(`📢 إعلان ${config.serverName}`, opt(i, 'message'), COLORS.main)] }); return reply(i, { embeds: [makeEmbed('تم الإعلان', 'تم إرسال الإعلان.', COLORS.ok)] }); }
  if (name === 'embed') { await post(i.channel_id, { embeds: [makeEmbed(opt(i, 'title'), opt(i, 'description'), COLORS.main)] }); return reply(i, { embeds: [makeEmbed('تم إنشاء الإيمبد', 'تم الإرسال.', COLORS.ok)] }); }
  if (name === 'poll') { const msg = await post(i.channel_id, { embeds: [makeEmbed('📊 تصويت', opt(i, 'question'), COLORS.info)] }); await api('PUT', `/channels/${i.channel_id}/messages/${msg.id}/reactions/✅/@me`); await api('PUT', `/channels/${i.channel_id}/messages/${msg.id}/reactions/❌/@me`); return reply(i, { embeds: [makeEmbed('تم إنشاء التصويت', 'تم إضافة التفاعلات.', COLORS.ok)] }); }
  if (name === 'giveaway') { const msg = await post(i.channel_id, { embeds: [makeEmbed('🎁 Giveaway', `الجائزة: **${opt(i, 'prize')}**
الفائزون: **${opt(i, 'winners') || 1}**
المدة: **${opt(i, 'minutes') || 60} دقيقة**
اضغط 🎉 للمشاركة.`, COLORS.gold)] }); await api('PUT', `/channels/${i.channel_id}/messages/${msg.id}/reactions/🎉/@me`); state.giveaways[msg.id] = { prize: opt(i, 'prize'), winners: opt(i, 'winners') || 1, endsAt: Date.now() + (opt(i, 'minutes') || 60) * 60000 }; save(); return reply(i, { embeds: [makeEmbed('تم بدء القيف أواي', `Message ID: \`${msg.id}\``, COLORS.ok)] }); }
  if (name === 'reroll') return reply(i, { embeds: [makeEmbed('🎲 Reroll', `تم تجهيز إعادة السحب للرسالة: \`${opt(i, 'message_id')}\``, COLORS.gold)] });
}
...
async function ticketCommand(i, name) {
  ...
  if (name === 'ticket-unclaim') return unclaimTicket(i);
  if (name === 'ticket-close') return closeTicket(i, opt(i, 'reason') || 'بدون سبب');
  if (name === 'ticket-transfer') return transferTicket(i, opt(i, 'type'));
...
async function unclaimTicket(i) {
  state.ticketMeta[i.channel_id] ||= {};
  state.ticketMeta[i.channel_id].assignedTo = null; save();
  await logTicket('🙋 إلغاء استلام', `الروم: <#${i.channel_id}>\nبواسطة: <@${i.member.user.id}>`, COLORS.warn);
  return reply(i, { embeds: [makeEmbed('🙋 تم إلغاء الاستلام', 'أصبحت التذكرة غير مستلمة.', COLORS.warn)] }, false);
}
async function transferTicket(i, type) {
  state.ticketMeta[i.channel_id] ||= {};
  state.ticketMeta[i.channel_id].type = type; save();
  const category = type === 'purchase' ? config.purchaseTicketCategoryId : config.supportTicketCategoryId;
  await api('PATCH', `/channels/${i.channel_id}`, { parent_id: category });
  return reply(i, { embeds: [makeEmbed('🔁 تم تحويل التذكرة', `النوع الجديد: **${ticketTypes[type].label}**`, COLORS.info)] }, false);
}
async function closeTicket(i, reason) {
  ...
  await uploadFile(config.ticketLogChannelId, `${closedId}.html`, transcriptHtml(transcript, closedId), 'text/html;charset=utf-8', { embeds: [makeEmbed('🌐 HTML Transcript', `التذكرة: **${closedId}**`, COLORS.info)] }).catch(console.error);
...
async function onChannelDelete(d) {
  const entry = Object.entries(state.tickets).find(([, channelId]) => channelId === d.id);
  if (entry) { delete state.tickets[entry[0]]; delete state.ticketMeta[d.id]; save(); await post(config.generalLogChannelId, { embeds: [makeEmbed('🧹 تنظيف تذكرة عالقة', `تم حذف بيانات التذكرة بعد حذف الروم: \`${d.id}\``, COLORS.warn)] }).catch(console.error); }
  await post(config.generalLogChannelId, { embeds: [makeEmbed('🗑️ حذف روم', `الروم: \`${d.id}\``, COLORS.danger)] }).catch(console.error);
}
async function onMessageDelete(d) { await post(config.generalLogChannelId, { embeds: [makeEmbed('🗑️ حذف رسالة', `الروم: <#${d.channel_id}>\nالرسالة: \`${d.id}\``, COLORS.warn)] }).catch(console.error); }
async function onMessageUpdate(d) { await post(config.generalLogChannelId, { embeds: [makeEmbed('✏️ تعديل رسالة', `الروم: <#${d.channel_id}>\nالرسالة: \`${d.id}\``, COLORS.info)] }).catch(console.error); }
async function onGuildMemberRemove(d) { await post(config.leaveLogChannelId, { embeds: [makeEmbed('👋 مغادرة عضو', `غادر العضو: <@${d.user.id}>\nالآيدي: \`${d.user.id}\``, COLORS.warn)] }).catch(console.error); }
async function onBanAdd(d) { await post(config.modLogChannelId, { embeds: [makeEmbed('🔨 Ban Log', `تم حظر: <@${d.user.id}>`, COLORS.danger)] }).catch(console.error); }
async function onBanRemove(d) { await post(config.modLogChannelId, { embeds: [makeEmbed('🔓 Unban Log', `تم إلغاء حظر: <@${d.user.id}>`, COLORS.ok)] }).catch(console.error); }
...
function connect() {
  ...
  ws.onmessage = async event => {
    ...
    if (payload.op === 10) {
      ...
      ws.send(JSON.stringify({ op: 2, d: { token, intents: 34311, properties: { os: process.platform, browser: 'velora-hub-termux', device: 'velora-hub-termux' } } }));
    }
    ...
    if (payload.t === 'GUILD_MEMBER_REMOVE') await onGuildMemberRemove(payload.d);
    if (payload.t === 'GUILD_BAN_ADD') await onBanAdd(payload.d);
    if (payload.t === 'GUILD_BAN_REMOVE') await onBanRemove(payload.d);
    if (payload.t === 'CHANNEL_DELETE') await onChannelDelete(payload.d);
    if (payload.t === 'MESSAGE_DELETE') await onMessageDelete(payload.d);
    if (payload.t === 'MESSAGE_UPDATE') await onMessageUpdate(payload.d);
  };
